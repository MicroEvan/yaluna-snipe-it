<?php

return array(
    'about'      	=> 'O oznakama statusa',
    'archived'      	=> 'Arhivirano',
    'create'      	=> 'Create Status Label',
    'color'      	=> 'Boja grafikona',
    'default_label' => 'Default oznaka',
    'default_label_help' => 'Ovo se koristi za osiguravanje da se najčešće korišćene oznake statusa pojavljuju pri vrhu okvira za odabir prilikom kreiranja/uređivanja imovine.',
    'deployable'      	=> 'Deployable',
    'info'      	=> 'Vrste oznaka statusa se koriste da opišu razne statuse u kojima može biti vaša imovina. Ona može biti na popravci, izgubljena/ukradena, itd. Možete napraviti novu oznaku statusa za zaduživu, na čekanju i arhiviranu imovinu prema vašem procesu rada. Za više infomracija, <a href="https://devs.yalunaictsolutions.com/yalu-assets/docs/overview#status-labels" target="_blank">pogledajte dokumentaciju<i class="fa fa-external-link"></i></a>.',
    'name'      	=> 'Naziv statusa',
    'pending'      	=> 'Pending',
    'status_type'   => 'Vrsta statusa',
    'show_in_nav'   => 'Show in side nav',
    'title'      	=> 'Oznake statusa',
    'undeployable'  => 'Undeployable',
    'update'      	=> 'Ažuriranje oznake statusa',
);
